...
export const AutoChart = (props: AutoChartProps) => {
  ...
  /** Will AVA The result of the obtained chart recommendation results and the merger of the model */
  const getMergedAdvices = (avaAdvices: Advice[]) => {
    ...
    const allAdvices = allChartTypes
      .map(chartTypeItem => {
        ...
        // If AVA Recommended list，Directly adopt the results from the recommended list
        if (avaAdvice) {
          return avaAdvice;
        }
        // If not，Generate a graph for it separately spec
  ...
  useEffect(() => {
    if (data && advisor) {
      ...
      // Merge the modelRecommended chart typesand ava Recommended chart types
    ...
    if (advices?.length > 0) {
      ...
      if (spec) {
        if (spec.data && ['line_chart', 'step_line_chart'].includes(chartTypeInput)) {
          // deal with ava Sorting issues with built-in line charts
          const dataAnalyzerOutput = advisor?.dataAnalyzer.execute({ data });
  ...
  return <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={'No suitable visual view yet'} />;
};
