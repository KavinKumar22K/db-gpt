          ...
          <Button
            ...
            onClick={async () => {
              ...
              if (contentType.includes('application/json')) {
                // in the case of JSON，Parse error message
                const reader = new FileReader();
                reader.onload = () => {
                  try {
                    const error = JSON.parse(reader.result as string);
                    message.error(error.err_msg);
                    // Display error messages on pages or notification systems
                  } catch (parseError) {
                ...
                // create URL and triggerdownload
    ...
    Table.EXPAND_COLUMN,
    {
      title: (
        <span className='w-[50px]'>
          ...
          <Tooltip placement='topLeft' title='View logs and ratings'>
            <InfoCircleOutlined />
          </Tooltip>
        </span>
      ),
        ...
        <div className='px-6 py-2 overflow-y-auto'>
          ...
          >
            <Form
              ...
              wrapperCol={{ span: 20 }}
            >
              <Form.Item name='scene_key' label='Scenetype' rules={[{ required: true }]}>
                <Select
                  options={[
                    {
                      value: 'recall',
              ...
              <Form.Item name='scene_value' label='Scene Parameters' rules={[{ required: true }]}>
                <Select
              ...
              <Form.Item name='parallel_num' label='Parallel parameters' rules={[{ required: true }]} initialValue={1}>
                <Input></Input>
              </Form.Item>
              <Form.Item name='datasets' label='Dataset' rules={[{ required: true }]}>
          ...
          <Modal
            title={isAddDataSet ? 'Add toDataset' : 'editDataset'}
          ...
          >
            ...
            >
              ...
              <Form.Item name='members' label='member'>
                <Select mode='tags' />
              </Form.Item>
              {isAddDataSet && (
